...
| ITEM — capture/creation | 0, 1 | Partial | Import and fully editable, image-derived name/type/color/season/formality defaults implemented; automatic isolated renditions, quick crop correction, expanded footwear/outerwear recognition, guided review progression, and metadata-synchronized default names covered; guided camera and brush-adjustable segmentation remain Phase 1 |
...
| Photo preparation, isolated garment renditions, quick crop, garment-focused perceptual colors, and accent suppression | `myCloset/ImageUtilities.swift`, `myCloset/QuickGarmentCropEditor.swift` | `ModelsAndImageTests` |
| Footwear/outerwear suggestions, season defaults, guided review, and synchronized import names | `myCloset/ClothingTypeDetector.swift`, `myCloset/ClosetImportReviewView.swift` | `ClothingTypeDetectorTests`, `MyClosetUITests` |
...
| Required per-piece import confirmation, metadata progression, and next-piece top reset | `myCloset/ClosetImportReviewView.swift`, `myCloset/ClosetView.swift` | `MyClosetUITests.testImportedPieceMustBeConfirmedAndCanBeCorrectedBeforeSaving` |
