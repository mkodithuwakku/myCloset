                ...
                VStack(spacing: 20) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 32, style: .continuous)
                            .fill(
                                LinearGradient(
                                    colors: [ClosetTheme.accentSoft, ClosetTheme.sage],
                        ...
                        VStack(spacing: 15) {
                            HStack(spacing: -12) {
                                ForEach(["person.crop.circle.fill", "person.crop.circle", "person.crop.circle.fill.badge.checkmark"], id: \.self) { icon in
                                    Image(systemName: icon)
                                        .font(.system(size: 44))
                                        .foregroundStyle(ClosetTheme.ink.opacity(0.74))
                                        .background(ClosetTheme.canvas, in: Circle())
                                        .overlay(Circle().stroke(ClosetTheme.canvas, lineWidth: 3))
                                }
                            }
                            Text("Outfit inspiration is next")
                                .font(.title2.weight(.bold))
                                .foregroundStyle(ClosetTheme.ink)
                            Text("The production feed needs accounts, secure cloud storage, blocking, reporting, and moderation. This prototype keeps the surface honest until those safeguards exist.")
                                .font(.subheadline)
                    ...
                    CardSurface {
                        VStack(alignment: .leading, spacing: 14) {
                            Label("Planned social slice", systemImage: "person.2.fill")
                                .font(.headline)
                            SocialPreviewRow(icon: "person.badge.plus", title: "Follow people", detail: "Find profiles by handle")
                            Divider()
                            SocialPreviewRow(icon: "photo", title: "See worn outfits", detail: "Photos and safe outfit snapshots")
                            Divider()
                            SocialPreviewRow(icon: "hand.raised.fill", title: "Stay in control", detail: "Report and block from every post")
                            Text("Likes and comments are intentionally not part of the product scope.")
                                .font(.caption)
...
private struct SocialPreviewRow: View {
    let icon: String
    let title: String
    let detail: String
